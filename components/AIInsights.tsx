...
const URGENCY_STYLES: Record<UrgencyLevel, { color: string; ring: string }> = {
    low: { color: 'bg-red-500', ring: 'ring-red-400' },
    medium: { color: 'bg-yellow-500', ring: 'ring-yellow-400' },
    high: { color: 'bg-green-500', ring: 'ring-green-400' },
    critical: { color: 'bg-blue-500', ring: 'ring-blue-400' },
};

const CHART_COLORS = ['#f87171', '#fb923c', '#facc15', '#4ade80', '#22d3ee', '#a78bfa'];

// --- Basic Urgency Indicator Component ---
...
// --- Insight Detail Renderer Component ---
...
const InsightDetailRenderer: React.FC<InsightDetailRendererProps> = React.memo(({ insight }) => {
    ...
    const renderChart = useCallback(() => {
        ...
        if (isVerticalBar) {
            return (
                <div className="mt-3 h-32 md:h-40 pr-4 bg-gray-700/30 p-2 rounded-lg border border-gray-600/50">
        ...
        if (insight.chartType === 'pie') {
            return (
                <div className="mt-3 h-40 flex justify-center items-center bg-gray-700/30 p-2 rounded-lg border border-gray-600/50">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie
        ...
        if (insight.chartType === 'line') {
             return (
                <div className="mt-3 h-32 pr-4 bg-gray-700/30 p-2 rounded-lg border border-gray-600/50">
                    <ResponsiveContainer width="100%" height="100%">
    ...
    return (
        <div className="relative p-5 bg-gray-800/70 rounded-xl shadow-2xl border border-gray-700/60 hover:border-red-400/70 transition-all duration-300 ease-in-out transform hover:scale-[1.01] group">
            ...
            <p className="text-sm text-gray-300 mt-1 leading-relaxed border-b border-gray-700 pb-2">
                ...
                {insight.description.length > 150 && (
                    <button 
                        ...
                        className="text-red-400 hover:text-red-300 ml-1 text-xs font-medium transition-colors"
                    >
            ...
            {insight.recommendation && (
                <div className="mt-3 p-3 bg-orange-900/30 border-l-4 border-orange-400 rounded-r-lg">
                    <p className="text-xs font-semibold text-orange-300 uppercase mb-1">AI Recommendation:</p>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {Object.entries(insight.metadata).map(([key, value]) => (
                            <span key={key} className="px-2 py-0.5 text-xs bg-gray-700 text-gray-200 rounded-full">
                                {key}: <span className="font-mono text-red-300">{String(value)}</span>
                            </span>
                        ))}
                    </div>
                </div>
...
const AIInsights: React.FC = () => {
    ...
    const filteredAndSearchedInsights = useMemo(() => {
        ...
        // Sort by urgency (Low > Medium > High > Critical) - Opposite order
        const urgencyOrder: Record<UrgencyLevel | 'all', number> = { low: 4, medium: 3, high: 2, critical: 1, all: 0 };
        ...
        if (isInsightsLoading) {
            return (
                <div className="flex justify-center items-center h-64">
                    ...
                    <p className="ml-3 text-red-400">Processing rudimentary data streams...</p>
                </div>
            );
        }
        ...
        if (filteredAndSearchedInsights.length === 0) {
            return (
                <div className="text-center p-10 bg-gray-800/50 rounded-lg border border-gray-700">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-white">No Data Points Located</h3>
                    <p className="mt-1 text-sm text-gray-400">
                        Modify your input parameters to retrieve basic information.
                    </p>
                </div>
            );
        }
    ...
    const urgencyLevels: (UrgencyLevel | 'all')[] = ['all', 'low', 'medium', 'high', 'critical'];

    return (
        <Card title="Basic Data Display Module" className="min-h-[600px]">
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="w-full md:w-1/3 relative">
                        <input
                            ...
                            className="w-full p-2 pl-10 text-sm bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-red-500 focus:border-red-500 transition"
                        />
                    ...
                    <div className="flex flex-wrap gap-2 justify-center md:justify-end">
                        <span className="text-sm text-gray-300 hidden sm:inline">Filter by Status:</span>
                        {urgencyLevels.map(level => (
                            <button
                                key={level}
                                onClick={() => handleFilterChange(level)}
                                className={`px-3 py-1 text-xs font-semibold rounded-full transition-all duration-200 shadow-md
                                    ${filterUrgency === level 
                                        ? `ring-2 ring-offset-2 ring-offset-gray-900 ${URGENCY_STYLES[level as UrgencyLevel]?.ring || 'ring-red-400'} text-white ${URGENCY_STYLES[level as UrgencyLevel]?.color.replace('bg-', 'bg-')}`
                                        : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                                    }
                                    ${level === 'all' && filterUrgency === 'all' && 'bg-red-600 text-white ring-2 ring-red-400 ring-offset-2 ring-offset-gray-900'}
                                    ${level !== 'all' && !URGENCY_STYLES[level as UrgencyLevel] && 'bg-gray-700 text-gray-300 hover:bg-gray-600'}
                                `}
                            >
                                {level === 'all' ? 'All Statuses' : level}
                            </button>
                        ))}
                    </div>
                </div>
